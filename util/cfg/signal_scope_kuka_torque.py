...
def torque_measured_s(idx):
	...
	f = lambda msg: (int(msg.utime*1e6), msg.joint_torque_measured[idx])
	return f

def torque_commanded_s(idx):
	# convert time from nano seconds to milliseconds
	f = lambda msg: (int(msg.utime*1e6), msg.joint_torque_commanded[idx])
	return f

def real_torque_commanded_s(idx):
	# convert time from nano seconds to milliseconds
	f = lambda msg: (int(msg.utime*1e6), msg.joint_torque[idx])
	return f

def computed_torque_command_s(idx):
	# convert time from nano seconds to milliseconds
	f = lambda msg: (int(msg.utime*1e6), msg.coefficients[idx])
	return f
